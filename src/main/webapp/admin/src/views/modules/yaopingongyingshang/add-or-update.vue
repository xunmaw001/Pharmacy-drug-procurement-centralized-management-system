...
export default {
  data() {
	...
	zhaopian : false,
      ...
      rules: {
          ...
          xiaoshoudaili: [
                { required: true, message: '销售代理不能为空', trigger: 'blur' },
          ],
          lianxidianhua: [
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `yaopingongyingshang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yaopingongyingshang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yaopingongyingshangCrossAddOrUpdateFlag = false;
                  this.parent.search();
