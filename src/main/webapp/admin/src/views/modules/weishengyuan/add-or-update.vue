<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="卫生院账号" prop="weishengyuanzhanghao">
          <el-input v-model="ruleForm.weishengyuanzhanghao" 
              placeholder="卫生院账号" clearable  :readonly="ro.weishengyuanzhanghao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="卫生院账号" prop="weishengyuanzhanghao">
              <el-input v-model="ruleForm.weishengyuanzhanghao" 
                placeholder="卫生院账号" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="管辖区域" prop="guanxiaquyu">
          <el-select v-model="ruleForm.guanxiaquyu" placeholder="请选择管辖区域">
            <el-option
                v-for="(item,index) in guanxiaquyuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="管辖区域" prop="guanxiaquyu">
	      <el-input v-model="ruleForm.guanxiaquyu"
                placeholder="管辖区域" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  data() {
	...
	zhaopian : false,
      ...
      rules: {
          weishengyuanzhanghao: [
                { required: true, message: '卫生院账号不能为空', trigger: 'blur' },
          ],
          mima: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='weishengyuanzhanghao'){
            this.ruleForm.weishengyuanzhanghao = obj[o];
	    this.ro.weishengyuanzhanghao = true;
            continue;
          }
          if(o=='mima'){
	    ...
	    this.ro.fuzerenxingming = true;
          ...
          if(o=='guanxiaquyu'){
            this.ruleForm.guanxiaquyu = obj[o];
	    this.ro.guanxiaquyu = true;
            continue;
          }
          if(o=='lianxidianhua'){
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              ...
              if (data && data.code === 0) {
                this.guanxiaquyuOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `weishengyuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.weishengyuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
