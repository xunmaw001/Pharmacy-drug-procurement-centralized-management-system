<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="所属区域" prop="suoshuquyu">
          <el-select v-model="ruleForm.suoshuquyu" placeholder="请选择所属区域">
            <el-option
                v-for="(item,index) in suoshuquyuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  data() {
	...
	zhaopian : false,
      ...
      rules: {
          weishengshizhanghao: [
                { required: true, message: '卫生室账号不能为空', trigger: 'blur' },
          ],
          mima: [
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/quyuxinxi/quyuming`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.suoshuquyuOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `weishengshi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.weishengshiCrossAddOrUpdateFlag = false;
                  this.parent.search();
