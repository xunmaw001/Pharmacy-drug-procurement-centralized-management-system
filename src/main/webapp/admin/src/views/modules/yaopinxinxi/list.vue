<template>
  <div class="main-content">
    ...
    <el-dialog
      ...
      width="800">
        <div id="yaopinkucunChart" style="width:100%;height:600px;"></div>
  ...
  methods: {
    ...
    chartDialog() {
      ...
      this.$nextTick(()=>{
        ...
        //  yaopinkucun
        // xcolumn ycolumn yaopinmingcheng yaopinkucun
        var yaopinkucunChart = this.$echarts.init(document.getElementById("yaopinkucunChart"),'macarons');
        this.$http({
            url: `value/yaopinxinxi/yaopinmingcheng/yaopinkucun`,
            method: "get"
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    // 使用刚指定的配置项和数据显示图表。
                    yaopinkucunChart.setOption(option);
                    //根据窗口的大小变动图表
                    window.onresize = function() {
                        yaopinkucunChart.resize();
                    };
                }
            }
        });
